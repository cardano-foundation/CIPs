...
- [CIP-0143](https://cips.cardano.org/cip/CIP-0143) (Interoperable Programmable Tokens) — currently `Inactive (incorporated into candidate CIP-0113)` — and its underlying problem statement [CPS-0003](https://cips.cardano.org/cps/CPS-0003) (Smart Tokens) use the same *deterministic-derived-script-address* pattern that TOA uses, but applied to a different problem: programmable transfer behaviour for fungible tokens via a per-user "smart token address," rather than NFT-controlled vault addresses. TOA and CIP-0143 are complementary rather than overlapping; they share an architectural pattern (a deterministic per-key script address) but target distinct use cases.
