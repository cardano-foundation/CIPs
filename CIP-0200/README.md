...
The urgent window of **5 samples** balances responsiveness and quote stability; see [Parameter calibration and tested ranges](#parameter-calibration-and-tested-ranges) for the supporting evidence.

The standard window of **10 processed-block summaries** balances smoothing against responsiveness to demand surges; see [Announcement semantics and the standard-window length](#announcement-semantics-and-the-standard-window-length) for the selection evidence.

The windows serve different purposes: the standard window supports service during demand surges, and the urgent window stabilises quotes. See [Window-removal experiment](#window-removal-experiment) for the supporting evidence.

See [Parameter calibration and tested ranges](#parameter-calibration-and-tested-ranges) and [Independent standard-lane target screen and confirmation](#independent-standard-lane-target-screen-and-confirmation) for the evidence behind the recommended `D = 16` and standard-lane target of `0.75`.
...
Using the Ranking Block's own capacity avoids an additional synthetic capacity parameter. See [Non-certificate window contribution](#non-certificate-window-contribution) for the supporting experiment and its limitations.

See [Announcement semantics and the standard-window length](#announcement-semantics-and-the-standard-window-length) for the evidence behind the window length and the exclusion of announcement entries.
...
The stepBound above is the largest single-update rise the update rule permits, so one step of headroom is exact rather than conservative. See [Fee headroom comparison](#fee-headroom-comparison) for the supporting experiment and its limitations.
...
[Parameter calibration and tested ranges](#parameter-calibration-and-tested-ranges) records that windows of 10–20 samples
...
[Parameter calibration and tested ranges](#parameter-calibration-and-tested-ranges) is a mechanism change
...
See [EB threshold and age escape](#eb-threshold-and-age-escape) for the supporting experimental evidence and its limitations.
...
##### Parameter calibration and tested ranges
