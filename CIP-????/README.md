...
2. Issuing governancve tokens - each user gets a vote on where the pooled assets will be delegated.
...
3. The spending script delegates the authorization of owner related actions to the address' staking credential - ie, the staking key must sign or the staking script must be successfully executed in the same tx.
