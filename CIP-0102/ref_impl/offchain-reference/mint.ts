...
/** Mints CIP-68 nfts specified in the sanitizedMetadataAssets parameter.
  ...
  */
  ...
  validator: Script,
	...
	const validTo = new Date();
	validTo.setHours(validTo.getHours(), validTo.getMinutes() + 5);
...
export async function mintWithCip25(lucid: Lucid, timestamp: number, nftData: NFTData) {
	...
	const policy = createTimelockedMP(lucid, timestamp, walletAddress);
	const policyId = lucid.utils.mintingPolicyToId(policy);

	const nftName = nftData.name.replace(/\s/g,'');
	const nftUnit1 = policyId + fromText(nftName);
	const nftUnit2 = policyId + fromText(nftName + "a");
