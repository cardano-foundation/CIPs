---
CIP: 193
Title: Auto-retirement of Abandoned Pools
Category: Ledger
Status: Proposed
Authors:
...
Implementors: []
Discussions:
    - Original PR: https://github.com/cardano-foundation/CIPs/pull/1230
Created: 2026-07-10
License: CC-BY-4.0
---
...
Appendix C, "Design Option: Stale Stake", of the [Shelley Delegation and Incentives Design Specification](https://github.com/intersectmbo/cardano-ledger/releases/latest/download/shelley-delegation.pdf) identified "stale stake" as a potential issue. It is classified as stake that is delegated to a pool that is no longer operating, but never submitted a deregistration transaction.  Because an unavailable pool can still be elected to produce blocks, the specification recognized stale stake as a threat to chain growth and proposed auto-retirement of persistently non-performing pools. That design option was not included in the initial Shelley release and consequently, stake pools today can be assigned blocks via the slot lottery even though their operators have abandoned them, hurting chain density.

This CIP introduces a governance-updatable protocol parameter, `spoActivity`, measured in epochs. A registered pool undergoes auto-retirement when it has neither produced a block on the adopted chain nor submitted an accepted pool registration certificate for `spoActivity` epochs. Producing a block or updating the registration renews the pool's expiration. The initial value recommendation is 73 epochs, approximately one year.

This CIP implements the deferred stale-stake objective through a deterministic activity period. Auto-retirement uses the existing pool-retirement behavior: the pool deposit is refunded according to the current rules, delegations to the pool are removed, and the pool must register again to return. Auto-retirement changes delegation state only.  It does not move, lock, or confiscate delegated ADA, which remains spendable and can be redelegated by its owner. A pool that produces no blocks generates no new pool rewards. The mechanism does not attempt to infer private leader assignments or punish temporary underperformance.

## Motivation: Why is this CIP necessary?

Appendix C, "Design Option: Stale Stake", of the Shelley Delegation and Incentives Design Specification begins from a direct chain-growth problem: an elected pool that has stopped operating will not produce its block. It proposed monitoring apparent pool performance and auto-retiring a pool when:
...
A small pool that does not produce a block within the activity period can remain registered by submitting its current pool parameters again. An abandoned pool whose operator no longer uses the cold key will eventually undergo auto-retirement.
...
- auto-retirement of obsolete pool and delegation state
...
- **Auto-retirement** is the application of the existing pool-retirement behavior to an abandoned pool without requiring a retirement certificate from its operator.
...
`poolExpiration` MUST have an entry for every registered pool. It records the epoch boundary at which the pool will undergo auto-retirement unless qualifying activity renews it.
...
The comparison is inclusive. Activity in epoch `A` with `spoActivity = P` sets the expiration to `A + P`. The pool remains registered through epoch `A + P - 1` and first undergoes auto-retirement at the transition into epoch `A + P`.

For an expiration epoch `E`, activity in epoch `E - 1` is the final activity that can reset the deadline. Qualifying activity in epoch `E` is too late, because the epoch transition has already occurred. The operator may instead register the pool again under the normal registration rules.

The extended pool-retirement transition MUST run before the existing snapshot transition at the boundary into epoch `E`. Consequently, the snapshot created at that boundary is the first snapshot that excludes an auto-retired pool and its delegations; the snapshot created at the preceding boundary is the last one that can include them. Leadership schedules and voting distributions already derived from earlier snapshots are unchanged and continue to have effect according to the existing snapshot timing.

### Pool auto-retirement
...
A retirement certificate schedules explicit retirement but is not a pool registration certificate. It does not renew `poolExpiration`. No relationship between `spoActivity` and `poolRetireMaxEpoch` is required.

Set union ensures that a pool appearing in both sets is processed only once. If explicit retirement and auto-retirement are both due at the same boundary, the existing pool-retirement transition applies once with its unchanged effects. If either is due at an earlier boundary, that transition removes the pool before the later boundary is reached.
...
- removing stake delegations to the auto-retired pool
...
The extended transition MUST also remove `poolExpiration[pool]`. This applies to both explicit retirement and auto-retirement, so the expiration map contains entries only for registered pools.

Auto-retirement MUST NOT refund a deposit twice or otherwise distinguish the value treatment of an auto-retired pool from an explicitly retired pool.

If an auto-retired pool registers again, it is a new registration under the existing rules. The appropriate pool deposit is required, and delegations deleted by auto-retirement are not restored. Delegators must submit new delegation certificates.
...
This gives every existing pool the same initial expiration without requiring historical activity to be reconstructed. No existing pool can undergo auto-retirement until 73 epochs after activation.
...
This avoids a bulk rewrite of registered-pool state when the parameter changes. It also follows the existing `drepActivity` pattern, which stores an expiry calculated when a DRep registers or updates rather than recalculating all DRep expirations when the parameter changes. The new interval takes effect gradually as pools renew.
...
Existing pool registration and retirement certificate formats are unchanged. The target era adds `spoActivity` to the protocol-parameter encoding and `poolExpiration` to pool state. Parameter values are updated through the existing governance mechanism; a substantive change to the activity or auto-retirement semantics requires a superseding CIP.

## Rationale: How does this CIP achieve its goals?

### Relationship to the Shelley stale-stake option

The Shelley design proposed auto-retiring a zero-performance pool only when its stake made several leader elections statistically likely. That test minimizes false positives for small pools, but it requires a probability threshold and depends on stake over the observation window.

This proposal retains a long observation period and gives every operator a deterministic alternative to producing a block: an on-chain registration update. Pool size therefore does not affect the operator's ability to avoid auto-retirement. A functioning but unlucky small pool can renew without waiting for a block, while a pool whose operator has abandoned both infrastructure and cold-key activity eventually leaves the registered set.

### Chain density

When stake is delegated to a registered but unavailable pool, the pool may win a private leader election and fail to produce the corresponding block. After the pool undergoes auto-retirement, its former delegations are no longer active stake. Future leader-election snapshots normalize over the remaining active stake under the existing consensus rules.
...
Pool auto-retirement removes delegations to the pool. Under existing snapshot timing, the pool consequently leaves the stake distribution used for subsequent SPO ratification. Votes already recorded under that pool credential may remain in governance-action history, but they carry no pool stake after the relevant distribution no longer includes the pool.

This effect is secondary to the chain-density goal. Auto-retirement is not a general SPO voting-activity rule, and producing a block is sufficient to keep a non-voting pool registered.

### Initial value of `spoActivity`

Seventy-three epochs is approximately one year on Cardano mainnet. It limits auto-retirement to pools that have shown no qualifying activity across a long operational window and remains substantially longer than ordinary maintenance outages or upgrade delays.

Lower values have two related operational effects. First, more pools, including pools with higher stake, can reach their expiration before producing a block and must submit a registration update to remain registered. Second, pools that are consistently too small to expect a block within the interval must renew more frequently as the interval is shortened. At 73 epochs, even a pool that produces no blocks needs at most one registration update per year. This is approximately four 90-day KES operational-certificate renewal cycles, each of which already requires access to the pool's cold key, so the registration update can be included in an existing cold-key maintenance workflow.
...
Storing `poolExpiration` performs the interval addition once, when qualifying activity occurs, and records the exact auto-retirement boundary. The epoch transition still scans registered pools, but each inactivity check is a direct comparison and query interfaces can expose the stored deadline without recomputing it.
...
- Auto-retirement is not slashing. No pool deposit, rewards, or delegated principal are confiscated.
- Auto-retirement does not move delegated ADA or select a new pool for it. The stake remains spendable but does not participate in staking or earn new pool rewards until its owner redelegates.
- Initializing every registered pool with the same activation-derived expiration can cause many abandoned pools to undergo auto-retirement at the same boundary.

## Path to Active

### Acceptance Criteria

- [ ] The formal ledger specification defines the parameter, state, activity renewal, auto-retirement, bootstrap, and parameter-update rules in this CIP.
...
- [ ] Automated tests demonstrate the specified state transitions, including last-eligible-epoch renewal, the explicit-retirement interaction, and the snapshot effect of auto-retirement, and characterize epoch-boundary performance when many pools expire together.
...
- [ ] Implement the new parameter, expiration state, bootstrap, activity updates, and auto-retirement transition.
