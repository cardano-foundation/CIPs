...
**A** - One problem is that there are already a lot of projects that are following this standard/CIP. If we want to add Schema.org, maybe we could move that towards a v2.0 so we can support both still.  
**M** - Another point on the image field with the double functionality. For a Standard you might expect everything to have (only) one purpose and that's why I propose to move it to thumbnail, so it only has one purpose (and use 'files' to store the actual image data).  
