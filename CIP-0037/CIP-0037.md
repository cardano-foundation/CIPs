...
Some applications might be interested in a smart contract aggregating signatures or keys . For this, we require $n$ group additions over $\mathbb{G}_1$ and $\mathbb{G}_2$ respectively, for $n$ the number of submitted signatures.

## Rogue-key attack resistant BLS
To prevent the rogue-key attack, one can use PoP of the secret keys. However, there are other mechanisms, as described [here](https://eprint.iacr.org/2018/483.pdf). For this, verification is as above, with the difference that key aggregation requires multiscalar multiplications of size $n$, in both $\mathbb{G}_1$ and $\mathbb{G}_2$, for $n$ the number of submitted signatures.

## ATMS
ATMS is a $k-of-t$ threshold signature scheme (meaning that we need at least $k$ signers to participate). Let $n\leq t - k$ be the number of signers that did not participate in the ATMS signature. ATMS verification proceeds as follows: 
...
* $t$ $\mathbb{G}_2$ additions
* $t\cdot \log_2(N)$ hash computations + 1 equality check
...
* Proof verification to a relation involving elements in $\mathbb{G}_1$ and $\mathbb{G}_2$. This verification does not require the pairing evaluation, but does require $\mathbb{G}_1$ and $\mathbb{G}_2$ additions and multiplications. 
* 2 pairing evaluations + equality check
...
* First part of batch poly commitment (7 scalar mults and 6 additions in $\mathbb{G}_1$)
* Fully batched poly commitment (7 scalar mults and 8 additions over $\mathbb{G}_1$)
* Group encoded batch verification (1 scalar mult over $\mathbb{G}_1$ )
* Batch-validate all evaluations (3 scalar mults and 4 additions over $\mathbb{G}_1$, plus two pairing evaluations + equality check)
