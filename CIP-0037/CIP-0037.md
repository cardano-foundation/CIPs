...
curve to the plutus language. We expose some candidate implementations, and describe clearly the benefits that this 
...
The reason for choosing BLS12-381 over BN256 curve is that the former is claimed to provide 128 bits of security, 
...
define a pairing over BLS12-381, where a pairing is a map: e:G_1 X G_2 -> G_T, which satisfies the following properties:
...
where G_1, G_2 and G_T are three distinct groups of order a prime q. Given that all three groups have the same 
...
* Group operations: 
    ...
    * `BLS12_381_G2_mult :: Integer -> Bls12381G2Element -> Bls12381G2Element`
...
* Deserialisation: 
  ...
  * `deserializeG2 :: ByteString -> Either BLSTError (Bls12381G2Element)`
* Equality functions:
...
This makes a total of 16 new functions and three new types. 
...
GT element is using the pairing function. Then these elements can only be used for operations among 
them (mult) and a final equality check. In other words, GT elements are only there to eventually
...
compute the miller loop, which already maps two points from G_1 and G_2 to G_T. Then we can use this result 
to perform the operations over these points (mults). Finally, when we want to check for equality, we invert 
one of the two points, and multiply the result by the other point. Only now we compute the final exponentiation,
...
* [Ethereum support for BLS12-381](https://eips.ethereum.org/EIPS/eip-2537). Not directly relevant as this is a 
...
computes the miller loop, and the equality check of GTs computes an inversion in GT, a multiplication, a final 
...
Some applications might be interested in a smart contract aggregating signatures or keys . For this, we require n 
group additions over G_1 and G_2 respectively, for n the number of submitted signatures.
...
that key aggregation requires multiscalar multiplications of size n, in both G_1 and G_2, for n the number of 
...
There are some additional t checks of lottery winning (similar to VRF). These are non crytpgraphic operations 
...
* Proof verification to a relation involving elements in G_1 and G_2. This verification does not require the 
* pairing evaluation, but does require G_1 and G_2 additions and multiplications. 
* 2 pairing evaluations + equality check

The complexity of the statement one can prove with anonymous credentials will be bounded by plutus computational 
...
respect to the additional gates and wires. In general, it is not expected that the number of custom gates surpasses 
the dozens. 
