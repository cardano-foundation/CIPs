...
result of `writeBits`, whose length is also $n$. We use $b[i]$ to refer to
...
< 0$ or $i \geq 8 \cdot n$, then `writeBits` fails. In this case, the
...
1. An error. In this case, we define the result of `writeBits` on such a
...
`writeBits` with data argument $b$ and change list argument $C^{\prime}$ 
as the result of `writeBits` with data argument $b_m$ and the change list
...
* Extend the shorter argument with the identity element (1 for
  `bitwiseLogicalAnd`, 0 otherwise) to match the length of the longer argument,
...
set representation; under padding semantics, the mask would only need to have $\Theta(k)$ 
length. This is noteworthy, as padding the mask would
...
can afford to exclude: we believe that match semantics are the choice, for
several reasons. Firstly, technically we can still have match semantics by using either
...
Haskell, we feel that excluding them as an option is both surprising and wrong. 
...
the changelist argument. This alone is good evidence for having `writeBits` instead;
