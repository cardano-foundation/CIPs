...
For `P1`, just knowing the public key is not sufficient to know the address (which we need to know if we want to prove ownership of a UTXO or transaction). To resolve this, first we (optionally) add to the header an extra field.
...
Both `address` and `address_pkey` are optional so that if the receiver knows this information or knows the mapping between the public key and the address, they can exclude them to reduce the payload size.

In the `v2` scheme, we need to specify the public key (since `address` only contains the `Blake2b-224` hash of the public key)
The `v3` scheme is similar but there can be both a staking and spending credential in base addresses, but it is trivial to check which of the two corresponds to `address_pubkey`.
As `v2` and `v3` addresses are incompatible, we can simply try both options until we reach the one that matches instead of requiring additional version data.
