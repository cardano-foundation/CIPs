...
A full illustrative document is published as a sibling file:
[`examples/ballot-definition.example.json`](./examples/ballot-definition.example.json).
It is a two-question ballot exercising the `single-choice` and `weighted`
methods, including per-question `contentHash` values, a complete `roleWeighting`
map, and a populated `ekklesia` extension block.

The two `contentHash` values in that example are `blake2b_256` of each question's
...
The normative JSON Schema (Draft 2020-12) is the sibling file
[`ballot-definition.schema.json`](./ballot-definition.schema.json) (`$id`:
`https://docs.ekklesia.vote/cip/hydra-voting-tokens/v0.3.0/ballot-definition.schema.json`).
A document that fails this schema is not a conforming ballot definition.
...
The normative JSON Schema (Draft 2020-12) is the sibling file
[`vote-evidence.schema.json`](./vote-evidence.schema.json) (`$id`:
`https://docs.ekklesia.vote/cip/hydra-voting-tokens/v0.3.0/vote-evidence.schema.json`).
The `vote-{tokenName}-v{version}.json` evidence files in the evidence directory
...
(600 DReps, 250 SPOs, 150 stakeholders) the published `results.json` is shown
in the worked example sibling file
[`examples/full-results.example.json`](./examples/full-results.example.json).
The single-choice Q1 received 900 explicit selections
...
The normative JSON Schema (Draft 2020-12) is the sibling file
[`results.schema.json`](./results.schema.json) (`$id`:
`https://docs.ekklesia.vote/cip/hydra-voting-tokens/v0.3.0/results.schema.json`).
The `results.json` file and every `proofs/{tokenName}.json` file in the evidence
...
definition in the sibling file
[`auditor-companion.schema.json`](./auditor-companion.schema.json) (`$id`:
`https://docs.ekklesia.vote/cip/hydra-voting-tokens/v0.3.0/auditor-companion.schema.json`).
