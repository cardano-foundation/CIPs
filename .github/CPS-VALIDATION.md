...
| No unquoted `?` values | Header fields must not have a bare `?` value (e.g. `CPS: ?`); it is invalid YAML and breaks GitHub's frontmatter rendering. Use a quoted `CPS: "?"` (or a word placeholder like `unassigned` / `pending`, which needs no quotes) until a number is assigned. |
...
| ----- | ----- | ---------------- |
| **CPS** | 1 | Positive integer (`1`, `42`) or a placeholder for unassigned: a single quoted `"?"`, `unassigned`, or `pending` (case-insensitive; `"??"`, `"???"`, or an unquoted `?` all fail — see file-level rules). No leading zeros. |
