...
2. Have `builtinIntegerToByteString` treat the requested digit count (argument 2
   in our description) as a maximum, not a minimum;
...
not adequately address the use case for finite field arithmetic, and in
...
An alternative possibility for requested digit count when calling
`builtinIntegerToByteString` (argument 2 in our description) is to instead treat
that as a _limit_, rather than a minimum. In such an implementation, if given
the number `123_456`, which needs three base-256 digits to represent, but with a
requested digit count of 2, `builtinIntegerToByteString` would fail, instead of
producing a result of length 3. One reason to do this is the finite field
arithmetic use case: in such situations, we know already how large our finite
field elements can ever get, and if we are ever asked to introduce one that's
larger than that, it can indicate that something went wrong, and prevent future
computations ending up in nonsensical states. This is particularly important
when such elements are being provided as user input, as there is no guarantee
that they are correctly-bounded, whether by accident or design. While
undoubtably useful, such a decision would make it difficult for users whose case
is similar to case 1: where the exact digit count of the number is not known
ahead of time. If some additional checks to ensure that too-large a
`BuiltinInteger` is not converted are needed, this can be achieved with our
implementation by using `lengthOfByteString` after conversion, then erroring out
if the size doesn't match what we expected: this requires only constant-time
operations, as `BuiltinByteString`s are counted. However, if the digit count was
treated as a limit, case 1 would become difficult: either the user has to guess
some huge limit (which is error-prone and inefficient), or they have to perform
a base-256 integer logarithm onchain (which is _extremely_ inefficient). We
believe that this would be an unacceptable tradeoff, thus rejecting the idea of
using the digit count as a maximum.
